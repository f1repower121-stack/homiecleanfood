...
interface LoyaltyConfig {
  ...
  tier_clean_eater: number
  tier_protein_king: number
  multiplier_homie: number
  multiplier_clean_eater: number
...
export default function AdmityTab({ darkMode = false }: Props) {
  const dm = darkMode
  ...
  const saveConfig = async () => {
    ...
    if (error) setConfigMsg('❌ Failed to save')
  ...
  const saveReward = async () => {
    ...
    else await supabase.from('loyalty_rewa').update(editReward).eq('id', editReward.id)
    setEditReward(null)
    fetchAll()
    ...
    <div>
        ...
        {sections.map(s => (
          <button key={s.key} onClick={() => setSection(s.key as any)}
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap transition-all
              ${section === s'bg-green-600 text-white shadow-sm' : `${muted} hover:bg-white/50`}`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className={`${card} border rounded-2xl p-4`}>
                <div className=9 rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center text-base mb-3`}>{s.icon}</div>
          ...
          <div className={`${card} border rounded-2xl p-5`}>
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm">
              <div className="space-y-2">
                ...
                <div className="flex jusfy-between"><span className={muted}>First order bonus</span><span className="font-medium">+{config.first_order_bonus} pts</span></div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justy-between"><span className={muted}>Max discount</span><span className="font-medium">{config.max_redeem_pct}% of order</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      {section === 'earn' && (
        <div className="space-y-5 max-w-2xl">
          ...
          <div className={`${card} border rounded-2xl p-5`}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="number" step="0.01" value={config.points_per_baht} onChange={e => setConfig({ ...config, points_per_baht: parseFloat(e.target.value) || 0 })} className={input placeholder="0.1" />
                <p className={`text-xs ${muted} mt-1`}>e.g. 0.1 = 1 point per ฿10</p>
              </div>
              <div className={`p-3 rounded-xl text-sm ${dm ? 'bg-gray-800' : 'bg-gray-50'}`}>
                <p className="font-medium">Preview: A ฿300 order earns <span className="text-green-600 font-bold">{Math.round(300 * config.points_per_baht)} points</span></p>
              </div>
            </div>
          </div>
          <div className={`${card} border rounded-2xl p-5`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                { key: 'first_order_bonus', label: 'First Order Bonus', icon: '🎉' },
                { key: 'birthday_bonus', label: 'Birthday Bonus', icon: '🎂' },
                { key: 'referral_bonus', label: 'Referral Bonus', icon: '📣' },
              ].map(f => (
                <div key={f.key}>
                  <label classNaxs font-medium ${muted} block mb-1`}>{f.icon} {f.label}</label>
          ...
          <div className={`${card} border rounded-2xl p-5`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className={`text-xs font-medium ${muted} block mb-1`}>Minimum points to redeem</label>
                <input type="number" value={config.min_redeem_points} onChange={e => setConfig({ ...config, min_redeem_points: parseInt(e.target.value) || 0 })} className={inputCls} placeholder="100" />
              </div>
              <div>
                <label className={`text-xs font-medium ${muted} block mb-1`}>1 point = ฿ value</label>
                <input typenumber" step="0.1" value={config.points_to_baht} onChange={e => setConfig({ ...config, points_to_baht: parseFloat(e.target.value) || 0 })} className={inputCls} placeholder="1.0" />
              </div>
              <div>
                <label className={`text-xs font-medium ${muted} block mb-1`}>Max discount (% of order total)</label>
                <input type="number" value={config.max_redeem_pct} onChange={e => setConfig({ ...config, max_redeem_pct: parseInt(e.target.value) || 0 })} className={inputCls} placeholder="30" />
              </div>
            </div>
            <div className={`mt-4 p-3 rounded-xl text-sm ${dm ? 'bg-gray-800' : 'bg-gray-50'}`}>
              <p className="font-medium">Preview: 100 points = <span className="text-green-600 font-bold">฿{(100 * config.points_to_baht).toFixed(0)} discount</span></p>
            </div>
          </div>
          <button onClick={saveConfig} disabled={configSaving} className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold hov:bg-green-700 disabled:opacity-50 transition-colors">
      ...
      {section === 'tiers' && (
        <div className="space-y-5 max-w-2xl">
          {configMsg && <div className={`p-3 rounded-xl text-sm font-medium ${configMsg.includes('✅') ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>{configMsg}</div>}
          <div className={`${card} border rounded-2xl p-5`}>
            <h3 className="font-semibold mb-1">Tier Thresholds</h3>
            <div className="space-y-4 mt-4">
              <div className={`flex items-center gap-4 p-4 rounded-xl ${dm ? 'bg-gray-800' : 'bg-gray-50'}`}>
                <span className="text-2xl">🌱</span>
                <div className="flex-1"><p className="font-semibold text-sm">Homie</p><p className={`text-xs ${muted}`}>Starting tier — 0 pts</p></div>
              </div>
           className={`flex items-center gap-4 p-4 rounded-xl border-2 border-lime-200 ${dm ? 'bg-gray-800' : 'bg-lime-50'}`}>
                <span className="text-2xl">🥗</span>
                <div className="flex-1">
                  <p className="font-semibold text-sm">Clean Eater</p>
                  <p className={`text-xs ${muted} mb-2`}>From this many points:</p>
                  <input type="number" value={config.tier_clean_eater} onChange={e => setConfig({ ...config, tier_clean_eater: parseInt(e.target.value) || 0 })} className={`w-32 border rounded-xl px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-green-400 ${dm ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-200 bg-white'}`} />
                </div>
                <div className="text-right">
                  <p className={`text-xs ${muted}`}>Multiplier</p>
                  <input type="number" step="0.1" value={config.multiplier_clean_eater} onChange={e => setConfig({ ...config, multiplier_clean_eater: parseFloat(e.taet.value) || 1 })} className={`w-20 border rounded-xl px-3 py-1.5 text-sm outline-none text-center ${dm ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-200 bg-white'}`} />
                  <p className={`text-xs ${muted} mt-0.5`}>× points</p>
                </div>
              </div>
              <div className={`flex items-center gap-4 p-4 rounded-xl border-2 border-amber-200 ${dm ? 'bg-gray-800' : 'bg-amber-50'}`}>
                <span className="text-2xl">👑</span>
                <div className="flex-1">
                  <p className="font-semibold text-sm">Protein King</p>
                  <p className={`text-xs ${muted} mb-2`}>From this many points:</p>
                  <input type="number" value={config.tier_protein_king} onChange={e => setConfig({ ...config, tier_protein_king: parseInt(e.target.value) || 0 })} className={`w-32 border rounded-xl px-3 py-1.5 text-sm outline-none focus:ring-2 focus:ring-green-400 ${dm ? 'bg-gray-700 border-gray-600 text-white' : 'border-y-200 bg-white'}`} />
                </div>
                <div className="text-right">
                  <p className={`text-xs ${muted}`}>Multiplier</p>
                  <input type="number" step="0.1" value={config.multiplier_protein_king} onChange={e => setConfig({ ...config, multiplier_protein_king: parseFloat(e.target.value) || 1 })} className={`w-20 border rounded-xl px-3 py-1.5 text-sm outline-none text-center ${dm ? 'bg-gray-700 border-gray-600 text-white' : 'border-gray-200 bg-white'}`} />
                  <p className={`text-xs ${muted} mt-0.5`}>× points</p>
                </div>
              </div>
            </div>
          </div>
          <button onClick={saveConfig} disabled={configSaving} className="w-full bg-green-600 text-white py-3 rounded-xl font-semibold hover:bg-green-700 disabled:opacity-50 transition-colors">
      ...
      {section === 'rewards' && (
        v className="space-y-5">
          <div className="flex items-center justify-between">
            <p className={`text-sm ${muted}`}>{rewards.filter(r => r.active).length} active rewards</p>
            <button onClick={() => { setIsNewReward(true); setEditReward({ active: true, reward_type: 'discount', emoji: '🎁', reward_value: 0 }) }} className="bg-green-600 text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-green-700 transition-colors">+ Add Reward</button>
          </div>
          {editReward && (
            <div className={`${card} border rounded-2xl p-5`}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  <label className={`text-xs font-medium ${muted} block mb-1`}>Reward Name *</label>
                  <input value={editReward.name || ''} onChange={e => setEditReward({ ...editReward, name: e.target.value })} claName={inputCls} placeholder="e.g. Free Drink" />
                </div>
                <div>
                  <label className={`text-xs font-medium ${muted} block mb-1`}>Emoji</label>
                  <input value={editReward.emoji || '🎁'} onChange={e => setEditReward({ ...editReward, emoji: e.target.value })} className={inputCls} placeholder="🎁" />
                </div>
                <div>
                ...
                <div>
                  <label className={`text-xs font-medium ${muted} block mb-1`}>Reward Type</label>
                  <select value={editReward.reward_type || 'discount'} onChange={e => setEditReward({ ...editReward, reward_type: e.target.value })} clase={inputCls}>
                ...
                <div>
                  <label className={`text-xs font-medium ${muted} block mb-1`}>Baht Value (฿)</label>
                ...
                <div className="col-span-2">
                  ...
                  <input value={editReward.description || ''} onChange={e => setEditReward({ ...editReward, description: e.target.value })} className={inputCls} placeholdeescription" />
                </div>
              </div>
              <div className="flex gap-2 mt-4">
                <button onClick={saveReward} disabled={rewardSaving} className="bg-green-600 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-green-700 disabled:opacity-50">{rewardSaving ? 'Saving...' : 'Save Reward'}</button>
      ...
      {section === 'customers' && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3 items-center">
            ...
            <button onClick={fetchAll} className={`text-xs border px-3 py-2.5 rounded-xl ${muted}`}>↻ Refresh</button>
          </div>
          <div className={`${card} border rounded-2xl overflow-hidden`}>
            {cusrs.filter(c => !custSearch || c.full_name?.toLowerCase().includes(custSearch.toLowerCase())).map(c => {
      ...
      {section === 'history' && (
        <div>
          <div className="mb-4 flex justify-between">
            ...
            <button onClick={fetchAll} className={`text-xs border px-3 py-1.5 rounded-xl ${muted}`}>↻ Refresh</button>
          </div>
          {redemptions.length === 0 ? (
            <div className={`text-center py-16 ${muted}`}><p className="text-3xl mb-2">📋</p><p>No redemption</p></div>
          ) : (
            <div className={`${card} border rounded-2xl overflow-hidden`}>
              {redemptions.map(r => (
